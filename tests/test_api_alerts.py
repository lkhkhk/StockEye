...
def test_auto_create_user_and_toggle_disclosure_alert(client, db_session):
    ...
    user = db_session.query(AppUser).filter(AppUser.telegram_user_id == test_telegram_user_id).first()
...
def test_set_price_alert_for_existing_user(client, db_session):
    ...
    user = AppUser(telegram_user_id=987654321, telegram_username="existinguser")
    db_session.add(user)
    db_session.commit()
...
def test_get_alerts_for_user(client, db_session):
    ...
    user = AppUser(telegram_user_id=11223344, telegram_username="alertlistuser")
    db_session.add(user)
    db_session.commit()
    db_session.refresh(user)
