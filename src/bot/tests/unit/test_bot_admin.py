...
async def test_admin_command(mock_update_context):
    ...
    assert "[관리자 전용 명령어]" in update.message.reply_text.call_args[0][0]
...
@patch('src.bot.handlers.admin.API_V1_URL', "http://localhost:8000/api/v1") # Patch API_V1_URL
async def test_admin_update_historical_prices_specific_stock_success(mock_http_post, mock_get_auth_token, mock_update_context):
    ...
    mock_response = MagicMock(spec=httpx.Response) # Use MagicMock for Response
    mock_response.status_code = 200
    mock_response.raise_for_status = MagicMock() # Sync mock
    ...
    mock_http_post.assert_awaited_with(
        f"http://localhost:8000/api/v1/admin/update_historical_prices", # Updated URL
...
async def test_admin_update_historical_prices_all_stocks_success(mock_http_post, mock_get_auth_token, mock_update_context):
    ...
    mock_get_auth_token.assert_awaited_with(chat_id)
    context.bot.send_message.assert_any_await(
        chat_id=chat_id, text=f"⏳ 과거 일별 시세 전체 갱신 요청 접수: 1990-01-01 ~ 2023-01-07. 작업이 완료되면 알림이 전송됩니다.", parse_mode='Markdown'
    )
    context.bot.send_message.assert_any_await(
        chat_id=chat_id, text="✅ 과거 일별 시세 갱신 작업이 성공적으로 트리거되었습니다.", parse_mode='Markdown'
    )
...
async def test_admin_update_historical_prices_invalid_date_format(mock_get_retry_client, mock_get_auth_token, mock_update_context):
    ...
    mock_put_response = MagicMock(spec=httpx.Response) # Sync response
    ...
    def mock_post_side_effect(url, *args, **kwargs):
        if "users/login" in url:
            mock_login_response = MagicMock(spec=httpx.Response) # Sync response
            mock_login_response.status_code = 200
            mock_login_response.json = MagicMock(return_value={"access_token": "fake_login_token"}) # Sync json
    ...
    # Use 3 arguments to trigger invalid date check in the 3-arg branch
    update.message.text = '/update_historical_prices 005930 invalid-date 2023-01-07'
...
async def test_admin_update_historical_prices_api_error(mock_http_post, mock_http_put, mock_get_auth_token, mock_update_context):
    ...
    mock_login_response = MagicMock(spec=httpx.Response)
    mock_login_response.status_code = 200
    mock_login_response.json = MagicMock(return_value={"access_token": "fake_token"})
    mock_login_response.raise_for_status = MagicMock()

    # Mock for the admin call
    mock_admin_response = MagicMock(spec=httpx.Response)
