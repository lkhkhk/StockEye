...
class TestNaturalHandler:
    ...
    @pytest.mark.asyncio
    @patch('requests.post') # MOCK: requests.post 함수
    @patch('requests.get') # MOCK: requests.get 함수
    async def test_natural_message_handler_stock_code_predict(self, mock_get, mock_post):
        ...
        # MOCK: requests.Response 객체 (predict mock)
        # MagicMock: requests.Response 객체를 모의합니다. 동기적으로 동작합니다.
        ...
        # mock_post (MagicMock) 호출 시 mock_post_response를 반환하도록 설정합니다.
        mock_post.return_value = mock_post_response

        await natural_message_handler(self.update, self.context)

        # mock_get (MagicMock)이 호출되지 않았는지 확인합니다.
        mock_get.assert_not_called()
        # mock_post (MagicMock)가 올바른 인자로 한 번 호출되었는지 확인합니다.
    ...
    async def test_natural_message_handler_stock_name_detail(self, mock_get, mock_post):
        ...
        # MOCK: requests.Response 객체 (symbols/search mock)
        # MagicMock: requests.Response 객체를 모의합니다. 동기적으로 동작합니다.
        ...
        # mock_get (MagicMock) 호출 시 mock_get_response를 반환하도록 설정합니다.
        ...
        # mock_get (MagicMock)가 올바른 인자로 한 번 호출되었는지 확인합니다.
    ...
    async def test_natural_message_handler_no_symbol_found(self, mock_get):
        ...
        # mock_get (MagicMock)가 다섯 번 호출되었는지 확인합니다.
        assert mock_get.call_count == 5
        # mock_get (MagicMock)가 올바른 인자로 호출되었는지 확인합니다.
    ...
    async def test_natural_message_handler_detail_api_failure(self, mock_get):
        ...
        # mock_get (MagicMock) 호출 시 다른 응답을 반환하도록 설정합니다.
        mock_get.side_effect = [mock_get_response_success, mock_get_response_failure]

        await natural_message_handler(self.update, self.context)

        # mock_get (MagicMock)가 두 번 호출되었는지 확인합니다. (첫 번째는 종목명 검색, 두 번째는 symbol로 상세 조회)
