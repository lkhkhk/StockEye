...
class TestNaturalHandler:
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.natural.session.get')
    @patch('src.bot.handlers.natural.session.post')
    async def test_natural_message_handler_stock_code_predict(self, mock_post, mock_get):
        ...
        mock_post.assert_called_once_with(f"{API_URL}/predict", json={"symbol": "005930"}, timeout=10)
        self.update.message.reply_text.assert_called_once_with("[예측 결과] 005930: 상승\n사유: 이동평균선이 정배열입니다.")

    @pytest.mark.asyncio
    @patch('src.bot.handlers.natural.session.get')
    @patch('src.bot.handlers.natural.session.post')
    async def test_natural_message_handler_stock_name_detail(self, mock_post, mock_get):
    ...
    async def test_natural_message_handler_no_symbol_found(self, mock_get):
        ...
        mock_get_response = MagicMock(spec=httpx.Response)
        mock_get_response.status_code = 200
        mock_get_response.json = AsyncMock(return_value=[]) # 종목 검색 결과 없음
        mock_get_response.raise_for_status = MagicMock(return_value=None)
        mock_get.return_value = mock_get_response

        await natural_message_handler(self.update, self.context)

        # 핸들러의 로직 변경에 따라 mock_get.call_args_list를 확인
        # '알', '수', '없는', '종목' 각 단어에 대해 검색 시도
    ...
    @patch('src.bot.handlers.natural.session.get')
    @patch('src.bot.handlers.natural.session.post')
    async def test_natural_message_handler_predict_api_failure(self, mock_post, mock_get):
        ...
        mock_post_response.json = AsyncMock(return_value={"detail": "500 Server Error"}) # json 메서드 mock
        mock_post_response.raise_for_status.side_effect = httpx.HTTPStatusError(
            message="500 Server Error", 
            request=httpx.Request("POST", f"{API_URL}/predict"), 
            response=httpx.Response(500, request=httpx.Request("POST", f"{API_URL}/predict"), json={"detail": "500 Server Error"}, text="500 Server Error")
        )
        mock_post.return_value = mock_post_response
        ...
        mock_post.assert_called_once_with(f"{API_URL}/predict", json={"symbol": "005930"}, timeout=10)
        self.update.message.reply_text.assert_called_once_with("예측 실패: 500 Server Error")

    @pytest.mark.asyncio
    @patch('src.bot.handlers.natural.session.get')
    @patch('src.bot.handlers.natural.session.post')
    async def test_natural_message_handler_detail_api_failure(self, mock_post, mock_get):
        """종목 상세 API 호출이 실패하는 경우 테스트"""
        self.update.message.text = "삼성전자"

        # symbols/search mock (첫 번째 호출은 성공, 두 번째 호출은 실패)
        mock_get_response_success = MagicMock(spec=httpx.Response)
        mock_get_response_success.status_code = 200
        mock_get_response_success.json = AsyncMock(return_value=[
            {
                "symbol": "005930",
                "name": "삼성전자",
                "market": "KOSPI"
        ...
        mock_get_response_success.raise_for_status = MagicMock(return_value=None)
        mock_get_response_failure = MagicMock(spec=httpx.Response)
        mock_get_response_failure.status_code = 500
        mock_get_response_failure.json = AsyncMock(return_value={"detail": "500 Server Error"})
        mock_get_response_failure.raise_for_status.side_effect = httpx.HTTPStatusError(
            message="500 Server Error", 
            request=httpx.Request("GET", f"{API_URL}/symbols/search"), 
            response=httpx.Response(500, request=httpx.Request("GET", f"{API_URL}/symbols/search"), json={"detail": "500 Server Error"}, text="500 Server Error")
        )
        ...
        self.update.message.reply_text.assert_called_once_with("종목 상세 조회 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요. (오류: 500 Server Error)")